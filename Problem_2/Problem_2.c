...
int main()
{
	...
	finput= fopen("input_problem_2.txt","r");
	foutput= fopen("output_problem_2.txt","w");
    int n;
    ...
    double e1=1.0e-7;   /*Termination parameters*/
...
{
    double e1=1.0e-8,e2=1.0e-8,e3=1.0e-8;		/*Termination parameters*/
...
double original_function(double* x,int n)	/*main objective function*/
{
    ...
    function = pow(sin(2*pi*x[0])/x[0],3)*sin(2*pi*x[1])/(x[0]+x[1]) ;
	return function;
}
...
double objective_function(double* x, int n, double R)	/*penalty function*/
{
    ...
    g1 = min(0,(-pow(x[0],2) + x[1] - 1)) ;
    g2 = min(0,(-1 + x[0] - pow(x[1]-4,2)) );
    penalty_function = -original_function(x,n) +  (R*pow(g1,2)) + (R*pow(g2,2));
