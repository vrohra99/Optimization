...
int main()
{
	...
	finput= fopen("input_problem_1.txt","r");
	foutput= fopen("output_problem_1.txt","w");
    int n;
...
double objective_function(double* x, int n, double R)   /*penalty function*/
{
    ...
    g1 = min(0,pow(x[0]-5,2) + pow(x[1]-5,2) - 100) ;
    g2 = min(0,-pow(x[0]-6,2) - pow(x[1]-5,2) + 82.81);
